...
They then gain an objective for their mentor to die and, when that occurs, inherit their mentor's secret identity objectives.
Then, they must ensure their mentor's objectives are completed.

If the mentor was a secret identity without any objectives, they gain a generic "failure" objective.

## Abilities
None.

## Gameplay
The Protégé serves as a twist on a general 'helper' type secret identity like the [Secretary](secretary.md).
...
Since choosing a mentor who does not have objectives counts as a failure, the Protégé must actively seek out and try to find individuals with secret identities that do have objectives.

Note that since the Protégé inherits their mentor's objectives instead of simply copying their secret identity, completion is preserved.
