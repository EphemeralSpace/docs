...
Of course, this is not a guide for *that,* as ES makes a lot of intentional deviances, but it should help inform you a bit if you use it for that purpose.
...
The best example of this principle in action is probably the energy gun. It has an obvious charge meter and place for the lasers to go through. Not only does this communicate the charge mechanic & futuristic nature of the gun well, it gives it a campy, simplistic quality that re-affirms the setting and the gun's origin (as a utilitarian gun on a space station with no "real" professional gun-users available.)
...
Inhands are also a notable example of this principle. Extremely frequently, items don't make any sense to be as big as they are (i.e EVA suits being as big as a helmet,) and spatially inconsistent between the 4 directions. Despite seeming illogical on an initial closer inspection, it ultimately saves time making sprites that would otherwise be extremely annoying to make, and ensures that the item is easily parsable at all times.

Also see Contrast for more information on this, technique-side.

### Retro-Futurism
It's the future, but old! There are CRT terminals everywhere, the armory still uses regular earthly shotguns with wooden stocks, and instead of smartphones, everyone carries around PDAs.
...
Cabinet projection is also usually great for readability & conveying what an object looks like while imposing less spritework, which is why it is often used on 1-directional structures & storage containers.
...
Your main takeaway from this should be to just use whatever perspective suits the situation best. Consistency is important to making a believable and good-looking game world, but readability and things looking good is *more* important. The game would not benefit if every hat was in a head-on perspective to be consistent with other items, or if the medibot was in head-on perspective to make it consistent with the other mobs.
...
### Contrast
Sprites should be easily visible in the situations that they are in commonly seen in. If it doesn't composite well in those situations, (i.e an item is hard to see on steel tiles,) then it's a problem with the sprite that should be rectified.
